...
- [~] IA copiloto (PRD 006) — sugestão por disciplina, perguntas, base recente/antiga declarada, persistência em `sugestoes` e métricas de aderência — *(código entregue e alinhado ao schema real de `sugestoes`, que já existe no banco desde a construção original; a migração `0116_sugestoes_ia.sql` só acrescenta um índice e ainda não foi aplicada; falta validar ponta a ponta no navegador)*
