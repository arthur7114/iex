...
> Fonte de verdade do escopo. A IA/copiloto (PRD 006) foi entregue na Fase 3 — ver docs/12-execution-roadmap.md. Todo o restante do MVP deve estar completo.

## Visão
App web B2B interno para elaborar, precificar, revisar, gerar (Word/PDF), enviar (e-mail) e acompanhar propostas de engenharia. IA é **copiloto** (não decisor). Princípios: rastreabilidade > automação; dados recentes (12m) priorizados; wizard guiado + revisão consolidada editável; documento padronizado mas editável.
...
- [x] Wizard de criação de proposta (10 etapas; IA opcional)
...
- [~] IA copiloto (PRD 006) — sugestão por disciplina, perguntas, base recente/antiga declarada, persistência em `sugestoes` e métricas de aderência — *(código entregue e alinhado ao schema real de `sugestoes`, que já existe no banco desde a construção original; a migração `0116_sugestoes_ia.sql` — só um índice — foi aplicada e validada em 03/08/2026 (30/30 checagens); falta validar ponta a ponta no navegador)*
